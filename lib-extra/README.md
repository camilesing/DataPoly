...
`build-extension.sh` 自动投放到这里——`build.sh` / `docker-maven-build.sh` 会先调用它：本地已有 `datapoly-extension/`
目录时即构建 backend（宿主机 JDK 25，低于 25 不可用）并投放本目录，脚本不做 git 拉取（仓库由使用者自行克隆/更新），
目录不存在则无操作（纯开源构建零影响）。本地一键入口仍是 `dev-local/dev.sh`（薄包装：注入 env.sh 后转发）。第三方额外 jar 也可手工投放。
