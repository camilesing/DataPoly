...
public class DataTaskController {
    ...
    @Operation(summary = "已注册的投递类型（sinkType 取值来源）")
    ...
    @Operation(summary = "试运行（不产生执行记录，返回 shaping 后的行）")
    ...
    @Operation(summary = "执行记录详情（前端轮询此接口获取完成状态与产物地址）")
    ...
    @Operation(summary = "取消执行（排队中直接取消；运行中的为协作式取消）")
