  ...
  data () {
    return {
      ...
      cmOptions: {
        ...
        mode: 'text/x-mysql',
        theme: 'darcula',
        lint: true,                     // lint warnings
        lineWrapping: true, // wrap long lines
        fontSize: 10,
        autofocus: true,
        matchBrackets: true,
        extraKeys: { "Tab": "autocomplete" },  // Tab triggers autocomplete
        hintOptions: { // autocomplete options
          completeSingle: false, // do not auto-complete on single match
          tables: {}
        }
      },
    }
  ...
  methods: {
    ...
    onCmCodeChange (cm, changeObj) {
      if (/^[a-zA-Z.]/.test(changeObj.text[0])) {
        // Only show hints for letters/dots, not spaces
        cm.showHint()
      }
    },
    ...
    handleClickTab (tab, event) {
      // el-tabs toggles panels via display:none; CodeMirror must recalculate
      // its layout after becoming visible to render and gain focus correctly.
      // $nextTick only guarantees DOM update, not a completed reflow,
      // so setTimeout defers to the next macrotask (after browser paint)
      setTimeout(() => {
        // Prefer instance from cmMapper
        var cm = this.cmMapper.get(this.currentTabName)
        // Fallback: get from $refs if missing in mapper
